...
The custom mobile robot, named Saminda Bot (SB), is a four-wheeled robot. The chassis is a box 0.4, 0.2, 0.05, while the wheels are cylinders with radius 0.055 and length 0.0375. Similar to Sec. \ref{subsec:udacity_bot}, the robot is equipped with a Hokuyo laser scanner and a camera. A Skid Steering Drive has been attached to front/back left/right wheels and configured via Gazebo. Fig. \ref{fig:saminda_bot} shows the structure of Saminda Bot, and the visual appearance is given in Fig.  \ref{fig:sb}.
